...
reader, which runs standalone as `python -m erph.inputs.dskp` and backs the
...
The structured output of `python -m erph.inputs.dskp` (and the input of `dskp` entries):
...
`python -m erph.inputs.dskp --pdf F --pages 35-45` extracts text with **pdfplumber** (an
...
… `T5`) used when a `dskp` handler configures no `file:`; `python -m erph.inputs.dskp --list`
