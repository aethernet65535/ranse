...
This reader recognises the source's day names (``DAY_HEADERS``, Sunday first)
and keeps **every day the source carries** — which days a template actually
has is the fillers' business, declared once by the profile in
...
from ranse.core.format import (NS, NS_R, parse_sheet_names, parse_sheet_rels,
...
def read_timetable_xlsx(zip_data, shared_strings, period_times=None):
    ...
    for row_num in sorted(grid.keys()):
        ...
        for col_num, cell_elem in cells.items():
            ...
            if val in DAY_HEADERS:
                day_col_map[col_num] = DAY_HEADERS[val]
