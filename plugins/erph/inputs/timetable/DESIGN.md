...
- **Header detection** — the first row containing a source day name (`Ahad` …
  ...
  ``Schedule`` carries by `DAY_HEADERS`: `Ahad` → `Sunday`, …) is the header;
...
**Risk 4** also covers the consumer side: `merge_periods` (in `domain.py`)
treats a **time gap as a new run** — two identical lessons separated by a gap
never merge. See
[`handlers/menu/DESIGN.md`](../../handlers/menu/DESIGN.md).
