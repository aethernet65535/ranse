...
from ranse.core.refs import _cell_ref, _resolve_path
from ranse.errors import ProfileError
from ranse.inputs.yaml import input_bases

from ...domain import DEFAULT_DAYS, SHEET_BY_DAY, merge_periods
...
def load_dskp_content(file_path):
    ...
    JSON: output from ``python -m erph.inputs.dskp`` (structured dict).
...
CLASS_BLOCK_SIZE = 31
CLASS_HEADER_ROW = 7  # row of the class header (the template's class title)
...
class DskpFiller:
    ...
    name = "dskp"
    phase = "fill"
    # The artifact day sheets checked before any write (the default day list
    # through the sheet mirror; a profile that overrides context.days beyond
    # these sheets still gets the per-entry "sheet not found" warning at
    # fill time).
    required_sheets = tuple(SHEET_BY_DAY[d] for d in DEFAULT_DAYS)
