...
load_jadual_config = fn("load_jadual_config")
ProfileError = fn("ProfileError")


def test_missing_minggu_records_is_a_profile_error(tmp_path):
    path = tmp_path / "cal.yaml"
    path.write_text("jadual: {}\n", encoding="utf-8")
    with pytest.raises(ProfileError) as exc:
        load_jadual_config(str(path))
...
def test_config_dir_is_added_next_to_the_file(tmp_path):
    ...
    cfg = load_jadual_config(str(path))
